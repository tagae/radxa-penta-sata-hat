...
DEBFILES := debian/control debian/postinst debian/prerm debian/templates \
			debian/conffiles debian/rockpi-penta.service
...
$(DEB): $(BIN) $(FONTS) $(ENVFILES) $(DEBFILES) rockpi-penta.conf
	rm -rf $(BUILDDIR)
	...
	# -- Binary --
	install -d              $(BUILDDIR)/usr/bin/rockpi-penta
	install -m 755 $(BIN)   $(BUILDDIR)/usr/bin/rockpi-penta/

	# -- Fonts (only the one used by the binary) --
	install -d $(BUILDDIR)/usr/bin/rockpi-penta/fonts
	install -m 644 fonts/DejaVuSansMono-Bold.ttf \
		$(BUILDDIR)/usr/bin/rockpi-penta/fonts/

	# -- Hardware env files --
	install -d $(BUILDDIR)/usr/bin/rockpi-penta/env
	install -m 644 env/*.env \
		$(BUILDDIR)/usr/bin/rockpi-penta/env/

	# -- Configuration --
	install -d          $(BUILDDIR)/etc
	install -m 644 rockpi-penta.conf $(BUILDDIR)/etc/

	# -- Systemd service --
	install -d          $(BUILDDIR)/lib/systemd/system
	install -m 644 debian/rockpi-penta.service \
		$(BUILDDIR)/lib/systemd/system/

	# -- Build the .deb --
	dpkg-deb --build -Z gzip $(BUILDDIR)
